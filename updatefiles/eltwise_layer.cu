...
template <typename Dtype>
__global__ void XorForward(const int nthreads, const Dtype* bottom_data_a,
    const Dtype* bottom_data_b, Dtype* top_data) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    top_data[index] = (bottom_data_a[index] == bottom_data_b[index]) ? Dtype(1) : Dtype(0);
  }
}
...
void EltwiseLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  case EltwiseParameter_EltwiseOp_MAX:
    ...
    break;
  case EltwiseParameter_EltwiseOp_XOR:
    XorForward<Dtype> <<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
